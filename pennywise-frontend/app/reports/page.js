...
export default function ReportsPage() {
  ...
  useEffect(() => {
    const fetchReportData = async () => {
      ...
      setIsLoading(true);
      try {
        const currentMonthParam = getCurrentYearMonthString();

        const overviewRes = await fetch(
          `${API_URL}/dashboard/current-month-overview`,
          { headers: authHeaders }
        );
        if (!overviewRes.ok) {
          ...
          throw new Error(
            errorData.message ||
              `Failed to fetch monthly overview. Status: ${overviewRes.status}`
          );
        }
        ...
        const categoriesRes = await fetch(
          `${API_URL}/dashboard/expense-breakdown?month=${currentMonthParam}`,
          { headers: authHeaders }
        );
        if (!categoriesRes.ok) {
          ...
          throw new Error(
            errorData.message ||
              `Failed to fetch top spending categories. Status: ${categoriesRes.status}`
          );
        }
      ...
      <div className="space-y-6 animate-fade-in">
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <CardTitle className="text-white">
                Current Month Overview -{" "}
                {monthlyOverviewData.currentMonthYearString || "Loading..."}
              </CardTitle>
            </div>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-green-400">
                  ${(monthlyOverviewData.totalIncome || 0).toLocaleString()}
                </p>
              </div>
              ...
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-red-400">
                  ${(monthlyOverviewData.totalExpenses || 0).toLocaleString()}
                </p>
              </div>
              ...
              <div className="text-center">
                ...
                <p
                  className={`text-2xl font-bold ${
                    (monthlyOverviewData.netIncome || 0) >= 0
                      ? "text-green-400"
                      : "text-red-400"
                  }`}
                >
              ...
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-purple-400">
                  {(monthlyOverviewData.savingsRate || 0).toFixed(1)}%
                </p>
              </div>
            </div>
          </CardContent>
        ...
        <Card className="glass-morphism border-gray-700">
          <CardHeader>
            <CardTitle className="text-white">
              Top Spending Categories -{" "}
              {monthlyOverviewData.currentMonthYearString || "Current Month"}
            </CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              {topCategoriesData.map((categoryItem, index) => {
                const percentage =
                  (monthlyOverviewData.totalExpenses || 0) > 0
                    ? (Math.abs(categoryItem.amount) /
                        monthlyOverviewData.totalExpenses) *
                      100
                    : 0;
                return (
                  <div
                  ...
                  >
                    ...
                    <p className="text-lg font-semibold text-white">
                      ${Math.abs(categoryItem.amount).toFixed(2)}
                    </p>
                  </div>
                );
              })}
                ...
                <CardHeader>
                  <div className="flex items-start justify-between">
                    <div className="flex items-center space-x-3">
                      <div
                        className={`w-12 h-12 bg-gradient-to-r ${report.color} rounded-xl flex items-center justify-center`}
                      >
          ...
          <CardContent>
            <div className="space-y-4">
              {[
                ...
                {
                  name: "Q4 2024 Quarterly Summary",
                  date: "2024-12-31",
                  size: "1.2 MB",
                },
                {
